...
fn try_main() -> i32 {
    ...
    const MAX_MEMBER_SIZE: u64 = 0x0008000000000000u64; // 2 PiB
    const MAX_VOLUME_SIZE: u64 = 0x4000000000000000u64; // 4 EiB
    let member_size = MAX_MEMBER_SIZE;
    let volume_size = 0u64;
    //   std::string default_output_filename;
    //   Mode program_mode = m_compress;
    let mut program_mode = Mode::Compress;
    //   Cl_options cl_opts;		// command-line options
    let cl_opts = ClOptions::default(); // command-line options
                                        //   bool force = false;
    ...
    //     );
    // } else {
    //     tmp = decompress(cfile_size, infd, cl_opts, pp, program_mode == Mode::Test);
    // }
    //       }
    //     catch( std::bad_alloc & )
